...
void toolbar(App* app) {
    ...
    constexpr float TOOLBAR_WIDTH = 320.0f;
    constexpr float TOOLBAR_HEIGHT = 48.0f;

    ImVec2 toolbar_pos(main_bounds.x - app->vd_min_x + (main_bounds.w - TOOLBAR_WIDTH) / 2.0f,
        main_bounds.y - app->vd_min_y + 8.0f);
    ImVec2 toolbar_size(TOOLBAR_WIDTH, TOOLBAR_HEIGHT);
    ...
    if (ImGui::Button("Region")) { /* TODO */
    }
    ImGui::SameLine();
    if (ImGui::Button("Window")) { /* TODO */
    }
    ImGui::SameLine();
    if (ImGui::Button("Screen")) { /* TODO */
    }

    ImGui::SameLine();
    ImGui::SeparatorEx(ImGuiSeparatorFlags_Vertical);

    float cancel_width = ImGui::CalcTextSize("Cancel").x + ImGui::GetStyle().FramePadding.x * 2.0f;
    ImGui::SameLine(TOOLBAR_WIDTH - cancel_width - ImGui::GetStyle().WindowPadding.x);
